...
import { getDatabase, ref, set, get, push, onValue, remove, query, orderByKey, orderByChild, orderByValue } from "https://www.gstatic.com/firebasejs/9.22.1/firebase-database.js"
...
addPopupExit.addEventListener("click", ()=>{
  ...
  addPopupForm.addEventListener("submit", (event)=>{
    ...
    get(ref(db, "levels/" + level.toLowerCase() + "/records/")).then(records=>{
        if(records.val())recordNum = Object.keys(records.val()).length
        
    })

    get(ref(db, "users/" + name)).then(user=>{
        if(!user.val()){
        ...
        set(ref(db, "users/" + name + "/records/" + level.toLocaleLowerCase()), {
            first: recordNum==0,
            name: level,
            video: video
        })
    
        set(ref(db, "levels/" + level.toLowerCase() + "/records/" + name.toLowerCase()), {
            name: name,
            video: video,
            recordNum: recordNum
        })
